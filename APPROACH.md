...
The scheduler isn't optimal, but it is kind of parametric and tweakable. The scheduling heuristics and the mux/gather split ratios are both controlled by a handful of numeric parameters (starvation thresholds, mux_count per level, etc.) that can be grid-searched cheaply after each structural optimization. This meant I could make a code-level change (e.g., adding deferred parity, extending mux to k=4) and then quickly sweep the parameter space to find the new sweet spot. A more principled scheduler would be more robust to shifts in kernel op mix and likely produce better results, but in practice this approach kept VALU utilization close to 100%.
